...
type Block struct {
	...
	Hacked       bool              // flag for when a malicious node has created an anti-block for this block
}
...
// map for knowing who the other malicious nodes
type MaliciousNodes struct {
	sync.RWMutex
	m map[int]bool
}
...
// malicious nodes
var maliciousNodes MaliciousNodes

// flag for when to take over the chain
var maliciousEnabled bool

// get map for overriding non-malicious nodes txn data: UUID -> [] of get keys
var currUUIDGets map[string][]string
...
func main() {
	...
	// check what nodes are malicious, enable if majority
	getMaliciousNodesInfo()
...
OuterLoop:
	for {
		...
		case block = <-unhashedBlockCh:
			...
			// create another block here to corrupt the gets by forking this block
			if maliciousEnabled && !block.Hacked {
				block.Hacked = true
				fakeBlock := createFakeBlock(block)

				logger.Println("FAKE block: ", fakeBlock)
				logger.Println("REAL block: ", block)

				// put real txn back on unhashed "todo" channel
				unhashedBlockCh <- block

				// start working on the fake block
				block = fakeBlock

				logger.Println("Start working on a FAKE block: ", block)
			}
...
func prettyPrintChain() {
	...
	for depth := 0; depth <= maxDepth; depth++ {
		...
		// // print a malicious info
		//       if maliciousEnabled {
		//           fmt.Printf("\n")
		//           for i := 0; i < len(blocks); i++ {
		//           	block := blocks[i]
		//               printWhiteSpace(whiteSpace)
		//               isMalicious := maliciousNodes.m[block.NodeID]
		//               fmt.Printf("| mal: %v   |", isMalicious)
		//           }
		//       }
...
func (c *ClientServer) Get(args *types.GetArgs, reply *types.GetReply) error {
	...
	if transaction, ok := store.Transactions[args.UUID]; ok {
		// keep track of gets so we can invalidate txns hashed by non-malicious
		currUUIDGets[args.UUID] = append(currUUIDGets[args.UUID], args.Key)
...
// malicious node only RPC
func (n *NodeServer) Malicious(senderNodeID *int, reply *int) error {
	*reply = nodeID
	return nil
}
...
func getMaliciousNodesInfo() {
	// Set all nodes to malicious except itself
	maliciousNodes = MaliciousNodes{m: make(map[int]bool)}
	for i := 1; i <= len(nodes.List); i++ {
		maliciousNodes.m[i] = false
	}
	maliciousNodes.m[nodeID] = true

	nodes.Lock()
	defer nodes.Unlock()

	// ping all nodes to see who is malicious
	var wg sync.WaitGroup
	for _, n := range nodes.List {
		if n.NodeIP == externalIP {
			continue
		}
		...
		go func(node types.Node) {
			var reply int
			call := node.NodeClient.Go("NodeServer.Malicious", &nodeID, &reply, nil)

			select {
			case <-call.Done:
				if call.Error == nil {
					logger.Println("Malicious node discovered with ID: ", reply)
					maliciousNodes.Lock()
					maliciousNodes.m[reply] = true
					maliciousNodes.Unlock()
				} else {
					logger.Println("Error testing for malicious node:", call.Error)
				}
				break
	...
	maliciousCount := 1
	for ID, malicious := range maliciousNodes.m {
		if ID != nodeID && malicious {
			maliciousCount++
		}
	}
	maliciousEnabled = (maliciousCount > (len(nodes.List) / 2))
	logger.Printf("Malicious Count: %v, Non-malicious Count: %v\n", maliciousCount, len(nodes.List)-maliciousCount)
	logger.Println("Malicious Enabled:", maliciousEnabled)

	currUUIDGets = make(map[string][]string)
}

// Creates a fake block that overwrites the keys for a given clients txns block
func createFakeBlock(blockToAbort *Block) *Block {
	logger.Printf("Creating fake block for %v\n", blockToAbort)

	keysToCorrupt := currUUIDGets[blockToAbort.UUID]

	newKeyVals := make(map[string]string)
	for _, key := range keysToCorrupt {
		newKeyVals[key] = randString(10)
	}

	fakeClientUUID, _ := tools.GetUUID()

	return &Block{
		PrevHash:    blockToAbort.PrevHash,
		TxnUpdates:  newKeyVals,
		NodeID:      nodeID,
		ValidateNum: 0,
		Validated:   false,
		Aborted:     false,
		UUID:        "123-MAL-" + fakeClientUUID,
		NoOp:        false,
	}
}
...
// stackoverflow.com/questions/22892120/how-to-generate-a-random-string-of-fixed-length-in-golang
func randString(n int) string {
	byteArr := make([]rune, n)
	for i := range byteArr {
		byteArr[i] = letters[rand.Intn(len(letters))]
	}
